...
-- paid out to whoever accepted the bounty if they land the killing blow
-- (see check_bounty_kill(), hooked from /mob/living/carbon/human/death()).
-- Single-accepter-at-a-time: once 'pending', no one else can also accept.
--
...
CREATE TABLE IF NOT EXISTS `ss13_bounties` (
  ...
  `reward`                INT NOT NULL,
  `status`                ENUM('open','pending','claimed','cancelled') NOT NULL DEFAULT 'open',
  `accepter_ckey`         VARCHAR(32) NULL,
