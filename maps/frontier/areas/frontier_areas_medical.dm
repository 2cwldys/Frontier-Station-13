/// MEDICAL_AREAS
/area/frontier/medical
	icon_state = "medbay"
	station_area = TRUE
	area_lighting = LIGHT_CLINICAL_COLORS
	holomap_color = HOLOMAP_AREACOLOR_MEDICAL
	area_blurb = "Various smells waft through the sick bay: disinfectants, various medicines, sterile gloves, and gauze. It's not a pleasant smell, but one you could grow to ignore."
	department = LOC_MEDICAL

/area/frontier/medical/paramedic
	name = "Paramedic Equipment Storage"
	horizon_deck = 3
	lightswitch = FALSE

/area/frontier/medical/reception
	name = "Reception"
	ambience = list('sound/ambience/signal.ogg')
	horizon_deck = 2

// Rad-shielded because its annoying as fuck
/area/frontier/medical/psych
	name = "Psych Office"
	icon_state = "medbay3"
	area_flags = AREA_FLAG_RAD_SHIELDED
	area_blurb = "Featuring wood floors and soft carpets, this room has a warmer feeling compared to the sterility of the rest of the medical department."
	horizon_deck = 2
	lightswitch = FALSE

/area/frontier/medical/pharmacy
	name = "Pharmacy"
	icon_state = "phar"
	horizon_deck = 2

/area/frontier/medical/cryo
	name = "Cryogenics"
	icon_state = "cryo"
	horizon_deck = 2

/area/frontier/medical/gen_treatment
	name = "General Treatment"
	icon_state = "medbay2"
	horizon_deck = 2

/area/frontier/medical/icu
	name = "Intensive Care Unit"
	icon_state = "medbay3"
	area_blurb = "The sounds of pumps and cooling equipment can be heard within the room."
	horizon_deck = 2

/area/frontier/medical/main_storage
	name = "Main Storage"
	icon_state = "medbay4"
	horizon_deck = 2
	lightswitch = FALSE

/area/frontier/medical/surgery
	name = "Operating Theatre"
	icon_state = "surgery"
	horizon_deck = 2

/area/frontier/medical/surgery/storage
	name = "Operating Theatre Storage"
	icon_state = "surgery"
	no_light_control = 1
	horizon_deck = 2
	lightswitch = FALSE

/area/frontier/medical/exam
	name = "Examination Room"
	icon_state = "exam_room"
	horizon_deck = 2
	lightswitch = FALSE

// Contains a player spawn area = rad-shielded
/area/frontier/medical/ward
	name = "Recovery Ward"
	area_flags = AREA_FLAG_RAD_SHIELDED
	icon_state = "patients"
	horizon_deck = 2

/area/frontier/medical/ward/isolation
	name = "Isolation Ward"
	area_blurb = "This seldom-used ward somehow smells sterile and musty at the same time."
	horizon_deck = 3

/area/frontier/medical/morgue
	name = "Morgue"
	icon_state = "morgue"
	ambience = AMBIENCE_GHOSTLY
	area_blurb = "Morgue trays sit within this room, ready to hold the deceased until their postmortem wishes can be attended to."
	horizon_deck = 1

/area/frontier/medical/equipment
	name = "Equipment Room"
	horizon_deck = 3
	lightswitch = FALSE

/area/frontier/medical/smoking
	name = "Smoking Lounge"
	area_blurb = "The smell of cigarette smoke lingers within this room."
	horizon_deck = 3

/area/frontier/medical/washroom
	name = "Head"
	horizon_deck = 3
	area_flags = AREA_FLAG_RAD_SHIELDED
	lightswitch = FALSE

/area/frontier/medical/hallway
	name = "Atrium"
	holomap_color = HOLOMAP_AREACOLOR_MEDICAL
	horizon_deck = 2

/area/frontier/medical/hallway/upper
	name = "Upper Atrium"
	horizon_deck = 3
