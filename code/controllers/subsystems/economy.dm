...
/datum/controller/subsystem/economy/proc/create_and_assign_account(var/mob/mob, var/override_name, var/funds, var/is_public)
	// Try to restore a previously saved account before creating a fresh one
	var/datum/money_account/money_account
	if(!override_name && mob.ckey && mob.real_name)
		money_account = restoreAccountFromPersistence(mob)
	if(!money_account)
		money_account = SSeconomy.create_account(override_name || mob.real_name, funds, null, is_public)
	money_account.ckey = mob.ckey
