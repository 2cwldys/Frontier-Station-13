...
/datum/controller/subsystem/ticker/Initialize(timeofday)
	...
	// Set state and mode now; actual game-start (SetRunLevel, records) is triggered
...
/**
 * Called by SSpersistence.Initialize() after all persistence data has loaded.
 * Completes the game-start sequence without spawning any characters.
 */
/datum/controller/subsystem/ticker/proc/start_persistent_world()
	// Wrap each step — persistence_ready MUST be set even if something fails
	try
		SSrecords.build_records()
	catch(var/exception/records_e)
		log_world("Persistent world: SSrecords.build_records() error (non-fatal): [records_e]")

	try
		Master.SetRunLevel(RUNLEVEL_GAME)
	catch(var/exception/runlevel_e)
		log_world("Persistent world: Master.SetRunLevel() error (non-fatal): [runlevel_e]")

	real_round_start_time = REALTIMEOFDAY
	round_start_time = world.time
	GLOB.persistence_ready = TRUE
	log_game("Persistent world: all systems ready — server is now open for players.")
