...
/datum/controller/subsystem/jobs/proc/EquipRank(mob/living/carbon/human/H, rank, joined_late = FALSE, spawning_at)
	...
	if(job)
		...
		var/has_saved_inventory = GLOB.config.sql_enabled && islist(GLOB.persistence_inventory_cache) && \
			GLOB.persistence_inventory_cache["[H.ckey]|[H.real_name]"]
		job.pre_equip(H)
		job.setup_account(H)
		job.after_spawn(H)
		if(!has_saved_inventory)
			//Equip job items — skipped when persistence has a saved loadout for this character.
		...
		// Apply persistence — health first, then inventory
