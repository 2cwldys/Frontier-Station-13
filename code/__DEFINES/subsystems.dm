...
#define INIT_ORDER_PERSISTENCE -10 // Persistence subsystem — runs after away maps(-2), ghost roles(-2.1), misc(-2.2), codex(-3) so the full world is built before DB state is applied
