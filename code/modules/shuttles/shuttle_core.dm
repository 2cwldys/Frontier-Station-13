 ...
 * actual ledger/materialization engine). This file's only remaining job is
 * the docking beacon itself: a portable, wrench+screwdriver-activated
 * machine that registers a real navigation landmark any ship's own
 ...
 * exoplanet landing already uses. Retrieve materializes a ship nearby;
 * Stash only becomes available once it has genuinely landed here under its
 * own power.
 */
