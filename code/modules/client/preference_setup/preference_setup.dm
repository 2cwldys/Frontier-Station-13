...
/datum/category_item/player_setup_item/proc/OnTopic(var/href,var/list/href_list, var/mob/user)
	// Block changes only for THIS specific character once it has entered the world.
	// When current_character == 0 (new character being created), always allow changes.
	if(pref && pref.current_character && GLOB.config.sql_saves && SSdbcore.Connect())
		var/datum/db_query/ot_q = SSdbcore.NewQuery(
