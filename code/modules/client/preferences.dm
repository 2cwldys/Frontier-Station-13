...
/datum/preferences
	...
	/// FALSE once a character has entered the world via PersistentAutoSpawn.
	var/can_edit_character = TRUE
	...
	// Neural lace — whether a neural lace is installed on this character at spawn
...
/datum/preferences/proc/open_load_dialog_sql(mob/user)
	var/slot_limit = get_slot_limit(user)
	var/dat = "<tt><center>"
	...
	for(var/ckey in GLOB.preferences_datums)
		...
		if(D == src)
			...
			dat += "<hr>"
			dat += "<b>[query.RowCount()]/[slot_limit] slots used</b><br>"
			if (query.RowCount() < slot_limit)
...
/datum/preferences/proc/delete_character_sql(var/client/C)
	...
	// Also wipe all persistence data for this character so it doesn't appear
	// in the persistent character selection menu or restore stale data.
	if(GLOB.config.sql_enabled && real_name && C.ckey)
		persistence_delete_character_data(C.ckey, real_name)
